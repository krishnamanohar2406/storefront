...
class CustomerSerializer(serializers.ModelSerializer):
    ...
    def get_address(self, customer):
        # A customer can have several saved addresses (see /store/addresses/);
        # this just surfaces the most recently added one for convenience.
        address = customer.address_set.order_by('-id').first()
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=Product
        fields=['id','title','slug','description','unit_price','original_price','discount_percent','inventory','brand','sizes','colors','material','avg_rating','num_ratings','collection','collection_name','images']
    def create(self, validated_data):
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=Order
        fields = ['id', 'customer', 'created_at', 'payment_status', 'delivered', 'delivered_at', 'razorpay_order_id', 'razorpay_payment_id', 'shipping_address', 'items']
...
class CreateOrderSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if address_id:
            if not Address.objects.filter(pk=address_id, customer__user_id=self.context['user_id']).exists():
                raise serializers.ValidationError({"address_id": "No address with the given id was found."})
    ...
    def save(self, **kwargs):
        with transaction.atomic():
            ...
            order = Order.objects.create(customer=customer, shipping_address_id=self.validated_data.get("address_id"))
